...
.confirmed:	lda	#BRAM_BANK		; Wipe the file from the BRAM
...
.copy_data:	sec				; Use the addr of the selected
		lda	tos_bram_fname + 0      ; filename as the destination.
		sbc	#6
		sta	<__bh + 0
		lda	tos_bram_fname + 1
		sbc	#0
		...
		clc				; Use the addr of the next BRAM
		...
		sec				; Calc length from next free
		lda	$6006			; addr in BRAM.
		sbc	<__ah + 0
		sta	<__ch + 0
		lda	$6007
		sbc	<__ah + 1
		sbc	#$20			; Rebase from $8000 to $6000.
		sta	<__ch + 1
		ora	<__ch + 0
		beq	.fill_free
...
.fill_free:	clc				; The next free location is
		lda	<__ch + 0		; destination + length.
		adc	<__bh + 0
		sta	<__bh + 0
		sta	$6006
		lda	<__ch + 1
		adc	<__bh + 1
		sta	<__bh + 1
		adc	#$20			; Rebase from $8000 to $6000.
		sta	$6007
		
		sec				; Calc length from next free
		lda	$6004			; to the end of BRAM.
		sbc	<__bh + 0
		sta	<__ch + 0
		lda	$6005
		sbc	<__bh + 1
		...
		lda	#$F3			; TAI instruction.
		sta	<__al
		lda	#<tos_zero		; Clear the free space.
		sta	<__ah + 0
		lda	#>tos_zero
		sta	<__ah + 1

		jsr	__al			; Execute TAI instruction.

		jsr	bm_bank_to_bram		; Copy the contents of BRAM
		bra	.finished               ; image from memory to BRAM.
...
.msg_warning:	db	"%y",11
		db	"  Please press %p1RUN%p0 to confirm that you",$0A
;		db	"   want to delete the file from BRAM!",$0A
		db	" want to delete ",$22,"%p2%10c%p0",$22," from BRAM!",$0A
		db	"%y",24
		db	"%x",11
